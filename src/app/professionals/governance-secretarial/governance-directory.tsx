...
export default function GovernanceDirectory({ people }: { people: Person[] }) {
  ...
  return <section className={styles.directory} id="find" aria-labelledby="find-title">
    <div className={styles.directoryHeading}><h2 id="find-title">Find a Governance & Secretarial Professional</h2><p>Explore published profiles by name, area of expertise or location.</p></div>
    <div className={styles.filters}>
      ...
      <a className={styles.searchButton} href="#advisors">View Professionals <Icon name="arrow" /></a>
      <button className={styles.resetButton} type="button" onClick={reset}>↻ Reset Filters</button>
    </div>
    <div className={styles.results} id="advisors"><div><span className={styles.eyebrow}>Meet our team</span><h2>Our Governance & Secretarial Professionals</h2></div><span role="status">{visible.length} {visible.length === 1 ? "profile" : "profiles"}</span></div>
